...
export default function App() {
  ...
  const loadProducts = async () => {
    ...
    try {
      ...
      showToast("Erro ao carregar produtos do Firebase.", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteProduct = async (id: string) => {
    if (
      !confirm(
        "Tem certeza que deseja apagar este produto? O cliente não o verá mais no app.",
      )
    )
      return;
    try {
  ...
  const persistStock = async (id: string, newQuantity: number) => {
    try {
      ...
      loadProducts(); // Reverte para o banco em caso de erro
    }
  };
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-[#F5F2EB] text-[#2A1610] font-sans overflow-hidden">
      ...
      {/* SIDEBAR */}
      <aside className="w-72 bg-[#2A1610] text-white flex flex-col shadow-2xl border-r border-[#B58E38]/20 shrink-0">
